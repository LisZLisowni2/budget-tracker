...
type Action = 
    | { type: 'UPDATE_FIELD'; field: keyof FormState; value: string | number | boolean | null }
...
export default function Transactions() { 
    ...
    const { transactions, loading: transactionLoading, handleAddTransaction, handleChangeTransaction, handleDeleteTransaction } = useTransactions()
    ...
    const handleSelect = (e: ChangeEvent<HTMLSelectElement>) => {
        const filtered = transactions.filter(transaction => transaction._id === e.target.value)
    ...
    return (
        <div className="flex items-center justify-between flex-col h-full *:p-4 max-lg:overflow-auto">
            ...
            <Modal activator={isChangeForm} header="Add transaction form" onClick={() => setStateChangeForm(!isChangeForm)}>
                <form className="p-8 m-4 *:w-full" onSubmit={handleChangeItem}>
                    <h1 className="text-3xl">Select Item</h1>
                    <select ref={selectRef} onChange={handleSelect} className="bg-white rounded-2xl p-4 my-6">
            ...
            <Modal activator={isDeleteForm} header="Add transaction form" onClick={() => setStateDeleteForm(!isDeleteForm)}>
            ...
            <div className="text-white flex max-md:flex-col justify-evenly w-full h-1/3 items-center">
                ...
                <Button text="Change transaction details" onClick={() => setStateChangeForm(!isChangeForm)}/>
                <Button text="Delete transactions" onClick={() => setStateDeleteForm(!isDeleteForm)}/>
            </div>
        </div>
    )
}